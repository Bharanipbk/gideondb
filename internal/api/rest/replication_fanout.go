...
func (s *Server) commitLeaderShardDelete(ctx context.Context, collection string, shardID uint32, namespace, id, acknowledgement, traceparent string) (int, bool, error) {
	s.replicationMu.Lock()
	defer s.replicationMu.Unlock()
	if s.rebalanceBarriers != nil {
		if err := s.rebalanceBarriers.ReleaseCommitted(s.currentMetadataEpoch()); err != nil {
			return 0, false, fmt.Errorf("release committed rebalance barriers: %w", err)
		}
		if barrier, frozen := s.rebalanceBarriers.IsFrozen(collection, shardID); frozen {
			return 0, false, fmt.Errorf("shard writes frozen for rebalance target epoch %d", barrier.TargetEpoch)
		}
	}
	var term uint64
	if s.replicationFactor > 1 {
		...
		if !ok {
			return 0, false, fmt.Errorf("metadata Raft status is required for replicated writes")
		}
		_, _, term = status.Status()
		if term == 0 {
			return 0, false, fmt.Errorf("metadata Raft term is not established")
		}
	}
	config, _, err := s.engine.DescribeCollection(collection)
	if err != nil {
	...
	if placement == nil || placement.LeaderID != s.nodeID {
		return 0, false, fmt.Errorf("local node is not the planned shard leader")
	}
	sequence, err := s.engine.DeleteShardWithSequence(collection, shardID, namespace, id)
	if err != nil {
		return 0, false, err
	}
	if s.replicationFactor == 1 {
	...
	results := make(chan replicaResult, len(placement.Replicas)-1)
	fanoutCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), 5*time.Second)
	defer cancel()
	for _, replicaID := range placement.Replicas[1:] {
		s.metrics.setReplicaLeaderSequence(collection, shardID, replicaID, sequence)
		peer, exists := peerByID[replicaID]
		if !exists {
			s.metrics.observeReplication("delete", "failure", collection, shardID, replicaID, sequence)
			results <- replicaResult{replicaID, fmt.Errorf("not discoverable")}
			continue
		}
		go func(replicaID string, peer cluster.Peer) {
			err := s.remoteReplicaDelete(fanoutCtx, peer, collection, shardID, sequence, namespace, id, term, traceparent)
			result := "success"
			if err != nil {
				result = "failure"
			}
			s.metrics.observeReplication("delete", result, collection, shardID, replicaID, sequence)
			results <- replicaResult{replicaID, err}
		}(replicaID, peer)
	}
	required, acknowledged, remaining := requiredAcknowledgements(acknowledgement, s.replicationFactor), 1, len(placement.Replicas)-1
	var failures []string
	// Drain every fanout before releasing replicationMu so a later WAL sequence
	// can never overtake this tombstone at a slow follower.
	for remaining > 0 {
		result := <-results
		remaining--
		if result.err == nil {
	...
	if acknowledged < required {
		return acknowledged, true, fmt.Errorf("replication acknowledgement level %s unavailable: acknowledged %d of %d replicas, require %d (%v)", acknowledgement, acknowledged, s.replicationFactor, required, failures)
	}
...
func (s *Server) remoteReplicaAppendOnce(ctx context.Context, peer cluster.Peer, collection string, shardID uint32, sequence uint64, records []core.Record, term uint64, traceparent string) (string, error) {
	...
	request.Header.Set("X-GideonDB-Leader-Node-ID", s.nodeID)
...
func (s *Server) remoteReplicaDelete(ctx context.Context, peer cluster.Peer, collection string, shardID uint32, sequence uint64, namespace, id string, term uint64, traceparent string) error {
	payload, err := json.Marshal(replicaAppendRequest{Sequence: sequence, ReplicationFactor: s.replicationFactor, Operation: "delete", Namespace: namespace, ID: id})
	if err != nil {
		return err
	}
	target := peer.SeedURL + "/v1/internal/replicas/" + url.PathEscape(collection) + "/" + strconv.FormatUint(uint64(shardID), 10) + "/append"
