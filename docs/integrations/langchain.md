...
existing GideonDB collection. The integration is loaded lazily: the core Python
...
store.add_texts(
    ["GideonDB stores dense vectors."],
    [{"kind": "guide"}],
    ids=["intro"],
)
documents = store.similarity_search("What does GideonDB store?", k=4)
```
...
text in payload, metadata in GideonDB metadata, and the record ID on returned
LangChain documents. `namespace` and GideonDB `filter` options can be supplied
